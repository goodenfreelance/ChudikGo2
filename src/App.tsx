          ...
          {isHeaderVisible ? (
            <div className="relative">
              <Controls
                ...
                onChangeGridTheme={setGridTheme}
                onToggleShowNodes={() => setShowNodes((n) => !n)}
                onAddRandomFood={handleAddRandomFood}
                onOpenEditor={handleOpenNewEditor}
                onOpenAnatomy={() => setIsAnatomyOpen(true)}
                onOpenLogs={() => setIsLogsOpen(true)}
                onAddPreset={handleAddPresetCreature}
                savedPresets={savedPresets}
                onSelectSavedPreset={(p) => {
                  soundFx.playEvolve();
                  const angle = determineCreatureHeadAngle(p.elements);
                  setPendingPlacement({
                    name: p.name,
