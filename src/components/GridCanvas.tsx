...
export const GridCanvas: React.FC<GridCanvasProps> = ({
  ...
  useEffect(() => {
    if (selectedCreatureId && canvasRef.current) {
      const target = (creatures || []).find((c) => c.id === selectedCreatureId);
      if (target) {
  ...
  }, [selectedCreatureId, focusTimestamp, creatures, zoom]);
  ...
  const gridToScreen = useCallback(
    (gridX: number, gridY: number): Point => {
      return {
        x: offset.x + gridX * CELL_SIZE * zoom,
        y: offset.y + gridY * CELL_SIZE * zoom,
      };
    },
    [offset, zoom]
  );
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const startX = Math.floor((-offset.x) / scaledCell) - 1;
      const endX = Math.ceil((width - offset.x) / scaledCell) + 1;
      const startY = Math.floor((-offset.y) / scaledCell) - 1;
      const endY = Math.ceil((height - offset.y) / scaledCell) + 1;
      ...
      for (let x = startX; x <= endX; x++) {
        const screenX = offset.x + x * scaledCell;
        ctx.moveTo(screenX, 0);
        ctx.lineTo(screenX, height);
      }
      for (let y = startY; y <= endY; y++) {
        const screenY = offset.y + y * scaledCell;
